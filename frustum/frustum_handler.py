...
def ensure_pixel_intrinsics(
    K: np.ndarray, normalized=False, height=None, width=None
) -> np.ndarray:
...
def sparse_grid_sample(
    ...
    if grid.ndim != 4 or grid.shape[-1] != 2:
        raise ValueError(
            f"Expected 4D grid [N,H_out,W_out,2], got shape={tuple(grid.shape)}"
        )
    if input.shape[0] != grid.shape[0]:
        raise ValueError(
            f"Batch size mismatch: input N={input.shape[0]}, grid N={grid.shape[0]}"
        )
    ...
    def _unnormalize(
        coord: torch.Tensor, size: int, align: bool | None
    ) -> torch.Tensor:
        if align:
            # [-1, 1] -> [0, size - 1]
    ...
    def _round_nearest(x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.where(x >= 0, torch.floor(x + 0.5), torch.ceil(x - 0.5)).to(
            torch.long
        )
    ...
    tgt_lin_all = torch.arange(h_out * w_out, device=device, dtype=torch.long).view(
        1, -1
    )
...
def fix_extrinsic_translation_axis_and_scale(
    T, trans_axis_order="xyz", flip_up_sign=False
):
...
def reproject_pixels_nd_to_nd(
    ...
    *,
    ...
    if pix_hw is None or depth_z is None or K_old is None or K_new is None:
        raise ValueError(
            "reproject_pixels_nd_to_nd: pix_hw, depth_z, K_old, K_new are required."
        )
    if T_old_w2c is None or T_new_w2c is None:
        raise ValueError(
            "reproject_pixels_nd_to_nd: must pass T_old_w2c and T_new_w2c "
            "(or legacy T_old_c2w / T_new_c2w aliases)."
        )
    ...
    if _K_old_single:
        ...
        invK_old = np.broadcast_to(
            np.linalg.inv(K_old[(0,) * len(lead)]), lead + (3, 3)
        )
    else:
...
class FrustumHandler:
    ...
    def _compute_view_change_for_points(points_world, source_w2c, query_w2c):
        ...
        ratio = np.linalg.norm(a, axis=-1) / np.maximum(
        ...
        phi = np.arctan2(u_b[:, 0], -u_b[:, 2]) - np.arctan2(
            u_a[:, 0], -u_a[:, 2]
        )
        phi = np.arctan2(np.sin(phi), np.cos(phi))
        theta = np.arcsin(np.clip(-u_b[:, 1], -1.0, 1.0)) - np.arcsin(
            np.clip(-u_a[:, 1], -1.0, 1.0)
        )
        return np.stack([ratio, phi, theta], axis=-1).astype(np.float32)

    def set_timing_reporter(self, timing_reporter) -> None:
        self._timing_reporter = timing_reporter

    @contextlib.contextmanager
    def _timing_scope(self, name: str, metadata: Optional[Dict] = None):
        reporter = getattr(self, "_timing_reporter", None)
        if reporter is None or not getattr(reporter, "enabled", False):
            yield self
            return
        try:
            with reporter.scope(str(name), metadata=metadata or {}):
                yield self
        except TypeError:
            with reporter.scope(str(name), **(metadata or {})):
                yield self
    ...
    def load_state(self, state: Dict, clip_num) -> None:
        ...
        self.depths = state.get("depths", [])[
            clip_num:
        ]  # 只保留最近的 depth_infer_fps 帧
        self.extrinsics = state.get("extrinsics", [])[
            clip_num:
        ]  # 只保留最近的 depth_infer_fps 帧
    ...
    def align_w2c_trajectory(
        self, A_w2c: np.ndarray, B_w2c: np.ndarray = None, eps: float = 1e-8
    ) -> np.ndarray:
        """
    ...
    def register_source_sequence(
        ...
        *,
        ...
        if force_using_input_extrinics and extrinsics is None:
            raise ValueError(
                "register_source_sequence: force_using_input_extrinics=True "
                "requires explicit `extrinsics`; got None."
            )
        if extrinsics is not None:
            extrinsics = np.asarray(extrinsics, dtype=float)
            if extrinsics.ndim != 3 or extrinsics.shape[1:] != (4, 4):
                raise ValueError(
                    f"extrinsics must have shape (T,4,4), got {extrinsics.shape}."
                )
            if extrinsics.shape[0] != len(frames):
                raise ValueError(
                    f"extrinsics length {extrinsics.shape[0]} does not match "
        ...
        if intrinsics is not None:
            if not force_using_input_extrinics:
                ...
                raise ValueError(
                    "register_source_sequence: `intrinsics` is only honoured "
                    "when `force_using_input_extrinics=True`."
                )
            ...
            if intrinsics.shape[0] != len(frames):
                raise ValueError(
                    f"intrinsics length {intrinsics.shape[0]} does not match "
                    f"frames length {len(frames)}."
                )
        ...
        if self._Kpix is None:
            self._Kpix = ensure_pixel_intrinsics(
                self._intrinsic, height=self._image_size[0], width=self._image_size[1]
            )
        ...
        if self._cache is None:
            ...
            batch_first_index = (
                int(expected_next) if start_index is None else int(start_index)
            )
        ...
        if depths is None:
            ...
            if _is_da3_compatible_estimator(self._depth_estimator):
                ...
                if not force_using_input_extrinics:
                    ...
                    base_extrinsics = np.eye(4)[None, ...].repeat(
                        extrinsics.shape[0], 0
                    )
                    ...
                    clip_num = 1 if len(extrinsics) > 1 else -1
                    ...
                    if cat_to_start:
                        ...
                        if self._fixed_intrinsics:
                            prepend_intrinsics = [
                                self._Kpix.copy() for _ in range(len(extrinsics))
                            ]
                ...
                print(f"使用 VideoDepthAnything 推理深度图")
        ...
        # ---- Normalize extrinsics to c2w ----
        def _fix_transform(Tm: np.ndarray) -> np.ndarray:
            return fix_extrinsic_translation_axis_and_scale(
                Tm,
                trans_axis_order=self._trans_axis_order,
                flip_up_sign=self._flip_up_sign,
            )

        if is_c2w:
            c2w_new = np.stack([_fix_transform(E) for E in extrinsics], axis=0)
        else:
            c2w_new = np.stack(
                [invert_pose(_fix_transform(E)) for E in extrinsics], axis=0
            )
        w2c_new = np.stack([invert_pose(C) for C in c2w_new], axis=0)
    ...
    def _frame_id_to_latent_id(
        ...
        *,
        ...
        if frames_per_latent == 1:
            ...
            latent_id = (
                0 if frame_id <= 0 else (1 + (frame_id - 1) // frames_per_latent)
            )
        return int(np.clip(latent_id, 0, total_latents - 1))

    @staticmethod
    def _frame_for_recent_first(memory_frames, frame_id: int):
        if memory_frames is None:
            return None
        frame = None
        if isinstance(memory_frames, dict):
            frame = memory_frames.get(int(frame_id))
        else:
            try:
                frame = memory_frames[int(frame_id)]
            except (IndexError, KeyError, TypeError):
                frame = None
        if frame is None:
            return None
        if isinstance(frame, torch.Tensor):
            frame_t = frame.detach().cpu()
            if frame_t.ndim == 4 and frame_t.shape[1] == 1:
                frame_t = frame_t[:, 0]
            if frame_t.ndim == 3 and frame_t.shape[0] in {1, 3, 4}:
                frame_np = frame_t[:3].permute(1, 2, 0).numpy()
            else:
        ...
        if frame_np.ndim != 3:
            return None
        if frame_np.shape[-1] != 3 and frame_np.shape[0] in {1, 3, 4}:
            frame_np = np.moveaxis(frame_np[:3], 0, -1)
        if frame_np.shape[-1] != 3:
            return None
        if frame_np.dtype != np.uint8:
            arr = frame_np.astype(np.float32, copy=False)
            if arr.size and arr.min() < 0.0:
                arr = (arr + 1.0) * 127.5
            elif arr.size and arr.max() <= 1.0:
                arr = arr * 255.0
            frame_np = np.clip(arr, 0, 255).astype(np.uint8)
        return frame_np

    def _build_recent_first_candidate_records(
        self,
        *,
        ...
        frame_source = (
            self.frames if memory_frames is None and self.frames else memory_frames
        )
        for frame_id in frame_candidates:
            ...
            latent_idx = self._frame_id_to_latent_id(
                fid,
            ...
            frame = self._frame_for_recent_first(frame_source, fid)
    ...
    @staticmethod
    def _detect_recent_first_keyframe_latents(
        w2c: np.ndarray,
        *,
        ...
        if w2c_arr.ndim != 3 or w2c_arr.shape[1:] != (4, 4):
            return []
        step = max(1, int(frames_per_latent))
        per_lat_w2c = w2c_arr[::step]
        if per_lat_w2c.shape[0] == 0:
            return []
        keyframes = [0]
        prev_R = per_lat_w2c[0, :3, :3]
        prev_t = per_lat_w2c[0, :3, 3]
        rot_thr_cos = float(np.cos(np.deg2rad(float(rot_thresh_deg))))
        max_interval = max(1, int(max_interval))
        for lat in range(1, int(per_lat_w2c.shape[0])):
            R = per_lat_w2c[lat, :3, :3]
            t = per_lat_w2c[lat, :3, 3]
            R_diff = R @ prev_R.T
            cos_a = float((np.trace(R_diff) - 1.0) * 0.5)
            cos_a = max(-1.0, min(1.0, cos_a))
            trans = float(np.linalg.norm(t - prev_t))
            if (
                cos_a < rot_thr_cos
                or trans > float(trans_thresh)
                or (lat - keyframes[-1]) >= max_interval
            ):
                keyframes.append(lat)
                prev_R = R
                prev_t = t
        return keyframes

    def _expand_recent_first_keyframe_candidates(
        self,
        frame_candidates: Sequence[int],
        *,
        w2c: np.ndarray,
        ...
        base: List[int] = []
        seen = set()
        total_frames = int(np.asarray(w2c).shape[0])
        for fid in frame_candidates:
            fid = int(fid)
            if fid < 0 or fid >= total_frames or fid in seen:
                continue
            base.append(fid)
            seen.add(fid)

        cfg = cfg or {}
        n_keyframes = int(cfg.get("n_candidates_keyframe", 0) or 0)
        if n_keyframes <= 0 or total_frames <= 0:
            return base

        # First implementation keeps latent_merge_4frames=False as the supported
        # path; merged latents need separate frame-id semantics validation.
        if int(frames_per_latent) != 1:
            return base

        key_lats = self._detect_recent_first_keyframe_latents(
            w2c,
            frames_per_latent=frames_per_latent,
            rot_thresh_deg=float(cfg.get("keyframe_rot_thresh_deg", 45.0)),
            trans_thresh=float(cfg.get("keyframe_trans_thresh", 1.0)),
            max_interval=int(cfg.get("keyframe_max_interval", 100)),
        )
        key_frames = [lat * int(frames_per_latent) for lat in key_lats]
        extras = [
            fid for fid in key_frames if 0 <= fid < total_frames and fid not in seen
        ]
        if len(extras) > n_keyframes:
            stride = len(extras) / float(n_keyframes)
            extras = [extras[int(i * stride)] for i in range(n_keyframes)]

        window = max(0, int(cfg.get("keyframe_neighbour_window", 0) or 0))
        expanded = list(base)
        for fid in extras:
            lat = fid // int(frames_per_latent)
            for nb_lat in range(max(0, lat - window), lat + window + 1):
                nb_fid = nb_lat * int(frames_per_latent)
                if 0 <= nb_fid < total_frames and nb_fid not in seen:
                    expanded.append(nb_fid)
                    seen.add(nb_fid)
        return expanded
    ...
    def _coerce_source_valid_masks(
        ...
        *,
        ...
        if tuple(masks.shape) != expected:
            raise ValueError(
                "source_valid_masks must have shape "
                f"{expected}, got {tuple(masks.shape)}."
            )
    ...
    def _coerce_memory_K(
        ...
        * Both None                   → returns None (handler falls back to
        ...
        if K is not None:
            raise ValueError(
                "Pass either ``K`` (legacy single (3,3) override) or "
                "``memory_K`` (per-frame), not both."
            )
        ...
        if arr.ndim == 3 and arr.shape[1:] == (3, 3):
            if arr.shape[0] != M:
                raise ValueError(
                    f"memory_K shape {arr.shape} must match w2c length M={M}."
                )
            return arr
    ...
    def _get_frame_intrinsic(
        ...
        if K_override is not None:
            ...
            if K.ndim == 3 and K.shape[1:] == (3, 3):
                ...
                return K[fid]
            raise ValueError(
                f"K_override must have shape (3,3) or (M,3,3), got {K.shape}"
            )
    ...
    def _build_group_representative_geometry(
        ...
        *,
        ...
        if mode == "second_frame":
            rep_frame_id = int(
                np.clip(
                    start + 1 if frame_ids.size >= 2 else start, 0, total_frames - 1
                )
            )
            ...
            rep_depth = np.where(
                depth_cnt > 0, depth_sum / np.maximum(depth_cnt, 1), 0.0
            ).astype(np.float32)
        else:
            raise ValueError(
    ...
    def _downsample_depth_to_latent(
        ...
        if reduce_mode == "block_min":
            ...
            raise ValueError(
                f"Unsupported reduce_mode={reduce_mode}. "
                "Use 'nearest', 'block_median', 'block_min' or 'cv2_area'."
            )
    ...
    def _clipped_proj_bbox(
        ...
        *,
        ...
        finite = (
            ...
            & (coords[:, 0] > sentinel_threshold)
            & (coords[:, 1] > sentinel_threshold)
        )
    ...
    def _pick_candidates_with_nms(
        ...
        def is_proximate(cand: Dict, picked_one: Dict) -> bool:
            if mode == "projection_iou":
                iou = FrustumHandler._bbox_iou_on_canvas(
                    cand.get("proj_bbox"), picked_one.get("proj_bbox")
                )
                return iou >= proj_thr
            if mode == "temporal":
                if temp_gap <= 0:
                    return False
                return (
                    abs(int(cand["frame_id"]) - int(picked_one["frame_id"])) < temp_gap
                )
            if mode == "pose":
                if pose_thr <= 0.0:
                    return False
                ...
                if t_a is None or t_b is None:
                    return False
                return (
                    float(np.linalg.norm(np.asarray(t_a) - np.asarray(t_b))) < pose_thr
                )
            raise ValueError(
                f"Unsupported candidate_nms_mode={mode!r}. Use "
                "'projection_iou', 'temporal', 'pose' or None."
            )
        ...
        for k, fid in enumerate(pool):
            ...
            if dl is None:
                dl = self._downsample_depth_to_latent(
                    ...
                    W_lat=Wd, H_lat=Hd, reduce_mode="nearest",
                )
    ...
    def _score_group_candidate_by_projection_iou(
        ...
        *,
        ...
        memory_K = self._get_frame_intrinsic(
            int(rep["frame_id"]), Ks=Ks, K_override=K_override
        )
        ...
        score = self._compute_canvas_iou_score(
            proj_hw_lat, (H_lat, W_lat), iou_mode=iou_mode
        )
        ...
        *,
        ...
        if frames_per_latent == 1:
            ...
            num_candidate_groups = (
                1 + (((total_frames - 1) + frames_per_latent - 1) // frames_per_latent)
                if total_frames > 1
                else 1
            )
        ...
        coarse_k = int(
            max(
                ...
                (
                    ...
                    if coarse_candidate_groups is not None
                    else candidates_per_query_group * 4
                ),
            )
        )
        ...
        if query_K is not None:
            ...
            if query_K_arr.shape == (3, 3):
                ...
                if query_K_arr.shape[0] != Q:
                    raise ValueError(
                        f"query_K shape {query_K_arr.shape} must match "
                        f"query_extrinsics length Q={Q}."
                    )
            else:
                raise ValueError(
                    f"query_K must have shape (3,3) or (Q,3,3), got "
                    f"{query_K_arr.shape}."
                )
        else:
            query_K_arr = None
        ...
        for qg in range(num_query_groups):
            ...
            if candidate_nms_mode is None:
                ...
                picked = self._pick_candidates_by_coverage_greedy(
                    scored, candidates_per_query_group
                )
            else:
    ...
    def query_hits_mode_old(
        ...
        for frame_candidates, keyframe_frame_candidates in zip(
            candidate_frame_ids, keyframe_candidate_frame_ids
        ):
    ...
    def select_candidates(
        ...
        *,
        ...
        if selection_mode in pose_selection_modes:
            ...
            num_query_groups = (
                num_query_frames // 4 if num_query_frames % 4 == 0 else num_query_frames
            )
            ...
            for qg in range(num_query_groups):
                query_array_idx = (
                    qg * 4 + query_reference_offset if num_query_frames % 4 == 0 else qg
                )
                ...
                if candidate_nms_mode is None:
                    ...
                    pool_topk = int(
                        round(
                            max(max_candidates, min(10, int(w2c.shape[0])))
                            * float(candidate_nms_pool_multiplier)
                        )
                    )
                ...
                if selection_mode == "pose_pool_temporal_earliest":
                    temporal_pool_k = int(
                        round(max_candidates * float(candidate_nms_pool_multiplier))
                    )
                    temporal_pool_k = min(
                        max(temporal_pool_k, max_candidates), len(ranked)
                    )
                    ranked_for_pick = sorted(
                        ranked[:temporal_pool_k], key=lambda item: int(item[0])
                    )
                else:
                ...
                if candidate_nms_mode is None:
                    ids = [
                        int(idx)
                        for idx, _angle, _dist in ranked_for_pick[:max_candidates]
                    ]
                else:
                    scored_pose = []
                        ...
                        scored_pose.append(
                            {
                                ...
                                "proj_bbox": None,
                                "rep_translation": np.asarray(
                                    cam_center, dtype=np.float32
                                ),
                            }
                        )
        ...
        if candidate_nms_mode == "coverage":
            ...
            # query_K explicitly, but inference/validation callers (e.g.
        ...
        if (
            total_latents is not None
            and frames_per_latent == 1
            and int(total_latents) != int(w2c.shape[0])
        ):
    ...
    def fuse_candidates(
        ...
        *,
        ...
        memory_frames=None,
        recent_first_cfg: Optional[Dict] = None,
        ...
        with self._timing_scope(
            ...
            K_override = self._coerce_memory_K(
                K=K, memory_K=memory_K, M=int(w2c.shape[0])
            )
        ...
        recent_first_geometry_cache: Optional[Dict[Tuple, Dict]] = (
            {} if fuse_mode == "recent_first_smooth" else None
        )
        for group_idx, frame_candidates in enumerate(candidate_frame_ids):
            ...
            with self._timing_scope(
                ...
                fused_out = self._fuse_group_latent_with_keyframes(
                    ...
                    recent_first_cfg=recent_first_cfg,
                    recent_first_geometry_cache=recent_first_geometry_cache,
        ...
        with self._timing_scope(
            ...
            output = torch.cat(queried_latents, dim=1)
            revgrid_output = (
                np.stack(queried_revgrids, axis=0) if return_revgrid else None
            )
            view_change_output = (
                np.stack(queried_view_changes, axis=0)
                if return_view_change
                else None
            )
            source_fid_output = (
                np.stack(queried_source_fids, axis=0)
                if return_source_frame_ids
                else None
            )
        extras: List = []
    ...
    def query_hits_mode_new(
        ...
        for backward compatibility (validation code still calls this entry
        ...
        keyframe_candidate_frame_ids = [
            [-1] * candidates_per_query_group for _ in candidate_frame_ids
        ]
    ...
    def _fuse_group_latent_with_keyframes(
        ...
        *,
        ...
        memory_frames=None,
        recent_first_cfg: Optional[Dict] = None,
        recent_first_geometry_cache: Optional[Dict[Tuple, Dict]] = None,
        return_view_change: bool = False,
        fuse_block_size: int = 1,
        return_source_frame_ids: bool = False,
        ...
        if int(fuse_block_size) == 2:
            if H_lat % 2 != 0 or W_lat % 2 != 0:
                raise ValueError(
                    "fuse_block_size=2 requires even (H_lat, W_lat); "
                    f"got ({H_lat}, {W_lat})."
                )
        ...
        if z_pref not in {"near", "far"}:
            raise ValueError(
                f"Unsupported zbuffer_depth_preference={zbuffer_depth_preference}. "
                "Use 'near' or 'far'."
            )

        neutral_view_change = np.array([1.0, 0.0, 0.0], dtype=np.float32)

        def _pack_fuse_result(
            fused_latent, revgrid=None, view_change=None, source_fid=None
        ):
            extras: List = []
            if return_revgrid:
        ...
        if fuse_mode == "recent_first_smooth":
            if return_view_change:
                raise ValueError(
                    "mosaic view-change PRoPE does not support "
                    "fuse_mode='recent_first_smooth' yet."
                )
            from frustum.mem_retrieval_v40_engine import run_recent_first_smooth

            with self._timing_scope(
                "materialize.fuse_candidates.group.recent_first.build_records",
                metadata={"candidate_count": len(frame_candidates)},
            ):
                frame_candidates = self._expand_recent_first_keyframe_candidates(
                    frame_candidates,
                    w2c=w2c,
                    cfg=recent_first_cfg,
                    frames_per_latent=frames_per_latent,
                )
                records = self._build_recent_first_candidate_records(
                    frame_candidates=frame_candidates,
                    depths=depths,
                    w2c=w2c,
                    Ks=Ks,
            ...
            need_debug_for_source_ids = bool(return_source_frame_ids)
            with self._timing_scope(
                "materialize.fuse_candidates.group.recent_first.engine",
                metadata={
                ...
                engine_out = run_recent_first_smooth(
                    query_K=query_K,
                    query_w2c=query_w2c,
                    ...
                    cfg=recent_first_cfg,
                    timing_scope=self._timing_scope,
                    timing_prefix="materialize.fuse_candidates.group.recent_first.engine",
                    candidate_geometry_cache=recent_first_geometry_cache,
                    return_debug=need_debug_for_source_ids,
                    source_valid_masks=source_valid_masks,
                )
                if need_debug_for_source_ids:
                    fused_latent, fused_revgrid, debug_info = engine_out
                else:
                    fused_latent, fused_revgrid = engine_out
                    debug_info = None

            extras: List = []
            if return_revgrid:
                extras.append(fused_revgrid)
            if return_view_change:
                # Guarded above; kept here for return-order completeness.
                extras.append(None)
            if return_source_frame_ids:
                # ``source_revgrid`` is (H_lat, W_lat, 3) with columns
                # (source_latent_idx, source_h, source_w). Map latent idx
                # -> raw frame id via the same mapping that built the
                # records, then coarsen to (H_lat//2, W_lat//2) at the
                # P-patch granularity by taking the top-left L-cell's
                # source. For ``fuse_block_size=2`` this is the canonical
                # block source-id; for ``fuse_block_size=1`` it is a
                # representative diagnostic.
                source_revgrid = (
                    debug_info["source_revgrid"]
                    if debug_info is not None
                    else np.full((H_lat, W_lat, 3), -1.0, dtype=np.float64)
                )
                latent_idx_L = source_revgrid[..., 0].astype(np.int64)
                latent_to_fid = {
                    int(r["latent_idx"]): int(r["frame_id"])
                    for r in records
                    if int(r.get("latent_idx", -1)) >= 0
                }
                src_fid_L = np.full((H_lat, W_lat), -1, dtype=np.int64)
                # Build a lookup table indexed by latent_idx.
                if latent_to_fid:
                    max_lat = max(latent_to_fid.keys())
                    lut = np.full((max_lat + 2,), -1, dtype=np.int64)
                    for k, v in latent_to_fid.items():
                        lut[k] = v
                    valid_idx_mask = (latent_idx_L >= 0) & (latent_idx_L <= max_lat)
                    safe_idx = np.where(valid_idx_mask, latent_idx_L, 0)
                    src_fid_L = np.where(valid_idx_mask, lut[safe_idx], -1)
                if int(fuse_block_size) == 2:
                    # Coarsen to P-patch via top-left of each 2x2.
                    source_fid_P = src_fid_L[::2, ::2].copy()
                else:
                    # block_size=1: still expose at P-patch granularity for
                    # API stability, also via top-left of each 2x2.
                    source_fid_P = src_fid_L[::2, ::2].copy()
                extras.append(source_fid_P)
            if extras:
                return (fused_latent, *extras)
            return fused_latent
                ...
                records.append(
                    {
                        ...
                        "depth": np.asarray(depths[fid], dtype=np.float32),
                        "w2c": np.asarray(w2c[fid], dtype=np.float32),
                        "K": self._get_frame_intrinsic(
                            fid, Ks=Ks, K_override=K_override
                        ),
                    }
                )
        ...
        def _splat_one(
            ...
            with self._timing_scope(
                ...
                z_buf = torch.full(
                    (N_pix,), float("inf"), device=device, dtype=torch.float32
                )
            ...
            with self._timing_scope(
                ...
                z_buf.scatter_reduce_(
                    0, flat_idx_v, z_v, reduce="amin", include_self=True
                )
            ...
            if winner_v.any():
                with self._timing_scope(
                    ...
                    src_flat_all = torch.arange(
                        N_pix, device=device, dtype=torch.long
                    ).view(H_lat, W_lat)
                    ...
                    canvas_xy_w_np = np.stack([j_q_f_w_np, i_q_f_w_np], axis=-1).astype(
                        np.float32
                    )
                    ...
                    if return_view_change:
                        ...
                        src_h = (src_flat.detach().cpu().numpy() // W_lat).astype(
                            np.int64
                        )
                        src_w = (src_flat.detach().cpu().numpy() % W_lat).astype(
                            np.int64
                        )
        ...
        def _splat_one_block(
            ...
            z_buf_P_flat = torch.full(
                (N_pat,), float("inf"), device=device, dtype=torch.float32
            )
            ...
            with self._timing_scope(
                ...
                z_v = torch.from_numpy(z_v_np).to(device=device)
                z_buf_P_flat.scatter_reduce_(
                    0, P_flat_v, z_v, reduce="amin", include_self=True
                )
            ...
            if winner_v.any():
                with self._timing_scope(
                    ...
                    a_h_grid = np.arange(H_lat - 1, dtype=np.int64)[:, None].repeat(
                        W_lat - 1, axis=1
                    )
                    a_w_grid = np.arange(W_lat - 1, dtype=np.int64)[None, :].repeat(
                        H_lat - 1, axis=0
                    )
                    winner_v_np = winner_v.detach().cpu().numpy()
                    ...
                    src_idx_t = torch.from_numpy(src_flat).to(
                        device=device, dtype=torch.long
                    )
                    tgt_idx_t = torch.from_numpy(tgt_flat).to(
                        device=device, dtype=torch.long
                    )
                    ...
                    valid_P_flat[
                        torch.from_numpy(target_P_flat_np).to(device=device)
                    ] = True
        ...
        if fuse_mode == "masked_mean":
            with self._timing_scope(
                ...
                fused_latent = self.masked_pixelwise_mean(
                    splat_list, invalid_masks=invalid_masks
                )
            if return_revgrid or return_view_change or return_source_frame_ids:
                with self._timing_scope(
                    ...
                    for rg, vc, sf in zip(
                        revgrid_list, view_change_list, source_fid_list
                    ):
        ...
        if fuse_mode == "fill_stop":
            with self._timing_scope(
                ...
                for splat, valid_mask, _zmap, rg, vc, sf in zip(
                    ...
                    if write.any():
                        fused_latent = torch.where(
                            write.unsqueeze(0).unsqueeze(0), splat, fused_latent
                        )
                        filled = filled | write
                        if return_revgrid or return_view_change or return_source_frame_ids:
        ...
        if fuse_mode == "fill_stop_zbuffer":
            with self._timing_scope(
                ...
                for splat, valid_mask, z_map, rg, vc, sf in zip(
                    ...
                    if z_pref == "near":
                        z_for_cmp = torch.where(
                            valid_mask, z_map, torch.full_like(z_map, float("inf"))
                        )
                    ...
                    if win.any():
                        fused_latent = torch.where(
                            win.unsqueeze(0).unsqueeze(0), splat, fused_latent
                        )
                        filled = filled | win
        ...
        raise ValueError(
            ...
            "Use 'masked_mean', 'fill_stop', 'fill_stop_zbuffer' or "
            "'recent_first_smooth'."
        )

    def masked_pixelwise_mean(
        self, xs, invalid_masks: Optional[Sequence[torch.Tensor]] = None
    ) -> torch.Tensor:
        """
        ...
        if len(xs) == 0:
            raise ValueError(
                "masked_pixelwise_mean requires at least one input tensor."
            )
        ref = xs[0]
        for i, x in enumerate(xs):
            if x.shape != ref.shape:
                raise ValueError(
                    f"All inputs must have the same shape. xs[0]={ref.shape}, xs[{i}]={x.shape}"
                )
        ...
        if invalid_masks is None:
            ...
            if len(invalid_masks) != len(xs):
                raise ValueError(
                    f"invalid_masks length must match inputs: {len(invalid_masks)} vs {len(xs)}"
                )
        ...
        for m in inv:
            ...
            if v.shape[0] != 1 and v.shape[0] != ref.shape[0]:
                raise ValueError(
                    f"Mask first dim must be 1 or C. Got mask shape {m.shape} -> broadcasted {v.shape}"
                )
            if v.shape[0] == ref.shape[0]:
                v = v[
                    :1
                ]  # take one channel; assumes invalid/valid consistent across channels
            valid_list.append(v)

        valid_stack = torch.stack(valid_list, dim=0).to(
            dtype=x_stack.dtype
        )  # (N,1,B,H,W) float
        ...
        out = torch.where(
            denom > 0, summed / denom.clamp(min=1), torch.zeros_like(summed)
        )
        return out
    ...
    def _compute_keyframe_labels_for_new_batch(
        self, c2w_new: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _select_candidates(
        ...
        if (
            ...
            and indices.size > max_candidates
        ):
    ...
    def _select_group_ids_by_vote(
        ...
        if keyframe_mask is None:
            ...
            if keyframe_mask.shape[0] != N:
                raise ValueError(
                    f"keyframe_mask length must match frame count {N}, got {keyframe_mask.shape[0]}."
                )
        ...
        # 候选组总数：组0 + ceil((N-1)/4)
        num_cand_groups = 1 + (((N - 1) + 3) // 4) if N > 1 else 1
        ...
        for qg in range(num_query_groups):
            ...
            for qi in range(q_start, q_end):
                ...
                for idx, angle, dist in candidates:
                    ...
                    if (
                        gid not in group_best_frame
                        or sub_score > group_best_frame[gid][3]
                    ):
            ...
            required_keyframes = max(
                0, min(required_keyframes, candidates_per_query_group)
            )
            ...
            keyframe_metrics = [
                m for m in gid_metrics if keyframe_mask[int(group_best_frame[m[0]][0])]
            ]
            ...
            picked_frame_ids: List[int] = [
                int(group_best_frame[gid][0]) for gid in picked_group_ids
            ]
    ...
    def find_best_match_by_angle_then_translation(
        ...
        *,
        ...
        # 归一化
        dirs_cand = dirs_cand / (
            np.linalg.norm(dirs_cand, axis=1, keepdims=True) + 1e-12
        )
        ...
        centers_cand = np.stack(
            [_cam_center(w2c_candidates[i]) for i in topk_indices], axis=0
        )  # (K, 3)
    ...
    def _rank_candidates_by_angle_then_translation(
        ...
        *,
        ...
        if query_frame_idx is not None and temporal_threshold is not None:
            valid_mask &= (int(query_frame_idx) - topk_indices) <= int(
                temporal_threshold
            )
