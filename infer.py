...
Each generated block is 21 latent frames = 80 new output frames; block k
consumes camera poses [1 + 84*k, 84*(k+1)]. Poses are clamped to the last
entry when the trajectory is shorter.

Weights are configured in paths.env (see paths.env.example).
"""
...
WEIGHTS = {
    ...
    "wan": ("WAN22_TI2V_5B_DIR", ["Wan2.2-TI2V-5B"],
            "Wan2.2-TI2V-5B base model directory"),
    "tokenizer": ("UMT5_TOKENIZER_DIR",
                  ["Wan2.2-TI2V-5B/google/umt5-xxl", "umt5-xxl"],
                  "umt5-xxl tokenizer directory"),
    "da3": ("DA3_MODEL_PATH", ["DA3NESTED-GIANT-LARGE-1.1"],
            "Depth-Anything-3 model directory"),
    "ckpt_first": ("CKPT_FIRST_PERSON", ["first-person.safetensors"],
                   "Matrix-Game-3.5 first-person checkpoint"),
    "ckpt_third": ("CKPT_THIRD_PERSON", ["third-person.safetensors"],
                   "Matrix-Game-3.5 third-person checkpoint"),
}
...
def load_camera(path, convention):
    ...
    if "intrinsics" not in z:
        sys.exit(f"ERROR: {path} must contain 'intrinsics' "
                 f"(N,4)|(4,)|(3,3) [fx,fy,cx,cy] in pixels")
    ...
    if intr.ndim == 1 and intr.shape[0] == 4:
        ...
        intr = np.tile(np.array([[fx, fy, cx, cy]], np.float32),
                       (extr.shape[0], 1))
    elif intr.ndim == 3 and intr.shape[1:] == (3, 3):
        intr = np.stack(
            [intr[:, 0, 0], intr[:, 1, 1], intr[:, 0, 2], intr[:, 1, 2]],
            axis=1)
...
def pad_poses(extr, intr, n_min):
    if extr.shape[0] >= n_min:
        return extr, intr
    print(f"[infer] camera has {extr.shape[0]} poses < {n_min}; "
          f"padding by repeating the last pose")
...
def write_refs(scene_dir, refs_dir):
    ...
    for i, name in enumerate(pngs):
        ...
        shutil.copyfile(os.path.join(refs_dir, name),
        ...
        if os.path.exists(src_mask):
            ...
            cv2.imwrite(os.path.join(img_dir, mask_name),
                        np.full(img.shape[:2], 255, np.uint8))
        h, w = img.shape[:2]
        rows.append({
            "ref_id": i, "frame_idx": i, "track_id": 1,
            "class_name": "person", "conf": 1.0,
            "bbox_xyxy": [0, 0, w, h], "crop_xyxy": [0, 0, w, h],
            "box_area": float(w * h), "mask_area": float(w * h),
            "temporal_bin_idx": i,
            "image_path": f"ref_images/{name}",
            "mask_path": f"ref_images/{mask_name}",
        })
    with open(os.path.join(dst, "candidates.jsonl"), "w",
              encoding="utf-8") as f:
        for row in rows:
...
def build_input_workspace(args, tmp_root):
    ...
    h, w = write_anchor_video(
        args.image, os.path.join(scene, "rgb", "video.mp4"))

    extr, intr = load_camera(args.camera, args.camera_convention)
    need = max(MIN_POSES, 1 + 84 * args.num_blocks)
    if extr.shape[0] < need:
        print(f"[infer] note: trajectory ({extr.shape[0]} poses) is shorter "
              f"than {need}; the tail will hold the last pose")
    extr, intr = pad_poses(extr, intr, MIN_POSES)
    np.savez_compressed(os.path.join(scene, "pose", "video.npz"), data=extr)
    np.savez_compressed(
        os.path.join(scene, "intrinsics", "video.npz"), data=intr)

    # Depth file must exist and parse; its values are never used for
    # generation (DA3 estimates depth at runtime).
    np.savez_compressed(
        os.path.join(scene, "depth", "video.npz"),
        frame_00000=np.full((h, w), 5.0, np.float32))
    json.dump({"format": "npz_float"},
              open(os.path.join(scene, "depth", "metadata.json"), "w"))
...
def build_index(ws, python_bin, env):
    ...
    cmd = [
        python_bin, "-m", "diffsynth.core.data.build_da3_video_index",
        "--camera_params_path", os.path.join(ws, "scenes"),
        "--prompt_path", os.path.join(ws, "caption"),
        "--output_path", index_path,
        "--workers", "1", "--worker_backend", "serial",
        "--quiet", "--overwrite",
    ]
    ...
    rows = con.execute(
        "SELECT clean_name, scan_error FROM records").fetchall()
    con.close()
    bad = [(n, e) for n, e in rows if e]
    if bad or not rows:
        sys.exit(f"ERROR: input scene was rejected by the index scanner: "
                 f"{bad or 'no records'}")
    return index_path
...
def run_generation(args, index_path, tmp_root, env):
    ...
    ckpt = resolve_weight(
        "ckpt_first" if args.person == "first" else "ckpt_third", args.ckpt)
    wan = resolve_weight("wan", args.wan_dir)

    cmd = [
        python_bin, "-m", "accelerate.commands.launch",
        "--num_processes", "1", "--num_machines", "1",
        "--machine_rank", "0", "--mixed_precision", "bf16",
        "--main_process_port", str(free_port()),
        os.path.join(REPO_DIR, "examples/wanvideo/pipeline/run_pipeline.py"),
        "--config", os.path.join(REPO_DIR, f"configs/infer_{person}.yaml"),
        "--model_id_with_origin_paths",
        f"{wan}:diffusion_pytorch_model*.safetensors,"
        f"{wan}:models_t5_umt5-xxl-enc-bf16.pth,{wan}:Wan2.2_VAE.pth",
        "--tokenizer_path", resolve_weight("tokenizer", args.tokenizer_dir),
        "--trained_dit", ckpt,
        "--dataset_index_path", index_path,
        "--val_dataset_index_path", index_path,
        "--num_val_batches", "1",
        "--num_validation_blocks", str(args.num_blocks),
        "--validation_sample_offset", "0",
        "--output_path", os.path.join(tmp_root, "raw"),
        "--log_dir_name", "run",
    ]
    if args.steps:
        cmd += ["--validation_num_inference_steps", str(args.steps)]
    if args.cfg_scale:
        cmd += ["--validation_cfg_scale", str(args.cfg_scale)]
    if args.seed is not None:
        cmd += ["--validation_seed", str(args.seed)]
    cmd += args.extra

    print("[infer] launching generation "
          f"({args.num_blocks} block(s) x 80 frames)...")
    subprocess.run(cmd, cwd=REPO_DIR, env=env, check=True)
    return os.path.join(tmp_root, "raw", "run", "validation")


def strip_gt_row(src, dst):
    """Drop the top (GT) row of the 3-row panel video.

    User inference has no ground truth — the pipeline's panel video stacks
    [GT | generation | mosaic memory] vertically; keep the lower two rows.
    """
    import cv2
    ...
    row_h = h // 3
    writer = cv2.VideoWriter(
        dst, cv2.VideoWriter_fourcc(*"mp4v"), fps, (w, h - row_h))
    while True:
        ok, frame = cap.read()
        if not ok:
            break
        writer.write(frame[row_h:])
    cap.release()
    writer.release()


def collect_outputs(val_dir, out_root):
    results = {}
    for f in sorted(os.listdir(val_dir)):
        path = os.path.join(val_dir, f)
        if not os.path.isfile(path):
            continue
        if f.endswith("_subject_ref_preview.jpg"):
            results["subject_ref_preview.jpg"] = path
        elif f.endswith("_history.mp4"):
            # pure generated rollout — this is the user-facing result
            results["result.mp4"] = path
        elif f.endswith(".mp4"):
            # panel video; GT row is stripped below
            results["memory_visualization.mp4"] = path
    for name, src in results.items():
        dst = os.path.join(out_root, name)
        if name == "memory_visualization.mp4":
            strip_gt_row(src, dst)
        else:
...
def main():
    ...
    ap.add_argument("--camera", required=True,
                    help="camera .npz (extrinsics_c2w + intrinsics)")
    ap.add_argument("--prompt", default="", help="prompt text")
    ap.add_argument("--prompt-file", default="", help="prompt text file")
    ap.add_argument("--caption", default="",
                    help="segment caption json (multi-block runs)")
    ap.add_argument("--refs", default="",
                    help="(third person) protagonist reference image dir")
    ap.add_argument("--num-blocks", type=int, default=1,
                    help="blocks to generate; each = 80 frames")
    ap.add_argument("--steps", type=int, default=0,
                    help="denoising steps (default from config: 25)")
    ap.add_argument("--cfg-scale", type=float, default=0.0,
                    help="CFG scale (default from config: 5.0)")
    ap.add_argument("--seed", type=int, default=None)
    ap.add_argument("--output", default=os.path.join(REPO_DIR, "outputs"))
    ap.add_argument("--name", default="",
                    help="run name (default: image stem + timestamp)")
    ap.add_argument("--camera-convention", choices=["c2w", "w2c"],
                    default="c2w")
    ap.add_argument("--keep-workspace", action="store_true")
    ap.add_argument("--ckpt", default="",
                    help="Matrix-Game-3.5 DiT checkpoint (default: "
                         "checkpoints/<person>-person.safetensors)")
    ap.add_argument("--wan-dir", default="",
                    help="Wan2.2-TI2V-5B dir (default: checkpoints/Wan2.2-TI2V-5B)")
    ap.add_argument("--tokenizer-dir", default="",
                    help="umt5-xxl tokenizer dir (default: checkpoints/umt5-xxl)")
    ap.add_argument("--da3-dir", default="",
                    help="DA3 model dir (default: checkpoints/DA3NESTED-GIANT-LARGE-1.1)")
    ...
    index_path = build_index(ws, sys.executable, env)
    val_dir = run_generation(args, index_path, tmp_root, env)

    results = collect_outputs(val_dir, out_root)
    if "result.mp4" not in results:
        sys.exit("ERROR: generation produced no result video "
                 f"(inspect {tmp_root})")
