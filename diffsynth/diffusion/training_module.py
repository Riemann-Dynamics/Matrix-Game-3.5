...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def add_lora_to_model(self, model, target_modules, lora_rank, lora_alpha=None, upcast_dtype=None):
        try:
            ...
            raise ImportError(
                "LoRA training requires the optional 'peft' package; plain "
                "inference does not. Install peft before enabling LoRA."
            ) from exc
    ...
    def parse_model_configs(self, model_paths, model_id_with_origin_paths, fp8_models=None, offload_models=None, device="cpu", loose_loading=False):
        ...
        if model_paths is not None:
            ...
            for path in model_paths:
                ...
                model_configs.append(ModelConfig(path=path, loose_loading=loose_loading, **vram_config))
        if model_id_with_origin_paths is not None:
            ...
            for model_id_with_origin_path in model_id_with_origin_paths:
                ...
                model_configs.append(ModelConfig(model_id=config.model_id, origin_file_pattern=config.origin_file_pattern, loose_loading=loose_loading, **vram_config))
