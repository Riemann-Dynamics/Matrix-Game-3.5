...
def _normalize_worker_backend(worker_backend):
    ...
    if backend not in {"process", "thread", "serial"}:
        raise ValueError(
            "worker_backend must be 'process', 'thread', or 'serial', "
...
def _done_child_scene_records(root_order, children, workers, max_scenes=None):
        ...
        for offset, record in enumerate(records):
            if record is None:
                if max_scenes is not None:
                    result["scene_records"] = [
                        local_scenes[name] for name in sorted(local_scenes)
                    ]
    ...
    remaining_children = [
        entry for entry in sorted_children if entry.path not in done_dirs
    ]
    return result, remaining_children, False
...
def _discover_da3_subtree(args):
    ...
    while stack:
        ...
        remaining_limit = (
            None
            if max_scenes is None
            else max(0, int(max_scenes) - len(local_scenes))
        )
        ...
        for record in done_result.get("scene_records", []):
            _add_done_record(local_scenes, result, record)
        if done_limit_reached or (
            max_scenes is not None and len(local_scenes) >= int(max_scenes)
        ):
            break
        ...
        stack.extend(
            entry.path
            for entry in sorted(children, key=lambda e: e.name, reverse=True)
        )
    result["scene_records"] = [local_scenes[name] for name in sorted(local_scenes)]
    return result
...
def _discover_da3_root(args):
    ...
    if workers > 1 and worker_backend != "serial":
        ...
        done_result, remaining_children, done_limit_reached = (
            _done_child_scene_records(
                root_order,
                children,
                workers,
                max_scenes,
        ...
        child_paths = [
            entry.path
            for entry in sorted(remaining_children, key=lambda entry: entry.name)
        ]
        if max_scenes is not None:
            remaining_limit = int(max_scenes) - len(
                done_result.get("scene_records", [])
            )
            if remaining_limit <= 0:
                return done_result
            if len(child_paths) == len(children) and not done_result.get(
                "scene_records"
            ):
                return _discover_da3_subtree(
                    (
        ...
        if child_paths:
            ...
            if len(child_paths) > 1:
                ...
                subtree_results = [
                    _discover_da3_subtree(
                        (root_order, camera_root, child_paths[0], None)
                    )
                ]
            subtree_results = sorted(
                subtree_results,
                key=lambda item: (
                    item.get("scene_records", [{}])[0].get("scene_dir", "")
                    if item.get("scene_records")
                    else ""
                ),
            )
    ...
    return _discover_da3_subtree(
        (root_order, camera_root, camera_root, max_scenes, workers, worker_backend)
    )
...
def _discover_da3_scenes(
    ...
    *,
    ...
    if max_scenes is not None or max_scenes_per_root is not None:
        ...
        for root_order, camera_root in enumerate(
            ...
            if root_limit is not None and root_limit <= 0:
                break
            result = _discover_da3_root(
                (root_order, camera_root, root_limit, workers, worker_backend)
            )
    ...
    root_args = [
        (idx, root, None, workers, worker_backend)
        for idx, root in enumerate(camera_roots)
    ]
    executor_cls = _executor_cls(worker_backend)
                ...
                if entry.name == "video.json":
                    if dir_scene_name == "caption":
                        scene_name = os.path.basename(
                            os.path.dirname(os.path.normpath(current))
                        )
                    else:
...
def _scan_one_scene(
    ...
    depth_format, depth_metadata_path, depth_metadata_json = _read_depth_metadata(
        depth_path
    )
    ...
    *,
    ...
    if worker_backend == "process":
        with ProcessPoolExecutor(
            ...
            futures = [
                pool.submit(_scan_one_scene_in_worker, scene) for scene in scene_values
            ]
            for future in tqdm(
                as_completed(futures),
                total=len(futures),
...
def _select_records_sql(columns):
    ...
    select_columns = [
        name if name in columns else f"NULL AS {name}" for name in base_columns
    ]
...
def _write_index(output_path, records, meta, overwrite):
    ...
    with sqlite3.connect(tmp_path) as conn:
        ...
        conn.execute(
            "CREATE INDEX idx_records_synthetic_path ON records(synthetic_path)"
        )
        conn.execute("CREATE INDEX idx_records_camera_root ON records(camera_root)")
        conn.execute(
            "CREATE INDEX idx_records_camera_root_order ON records(camera_root_order)"
        )
...
def build_da3_video_index(
    *,
    ...
    _debug(
        f"scene discovery done: unique_scenes={len(scenes)} "
        f"duplicates_in_new_roots={duplicate_count}",
        verbose=verbose,
    )
    ...
    if append:
        ...
        duplicate_count += append_duplicate_count + int(
            old_meta.get("duplicate_clean_name_count", 0) or 0
        )
    ...
    meta = {
        ...
        "max_scenes_per_root": (
            "" if max_scenes_per_root is None else int(max_scenes_per_root)
        ),
...
def load_da3_video_index_records(
    index_path, *, only_complete=True, min_frame_count=None
):
    with sqlite3.connect(index_path) as conn:
        conn.row_factory = sqlite3.Row
