...
class FrameSamplerByRateMixin:
    ...
    def get_available_num_frames(self, reader):
        ...
        duration = (
            ...
            else total_original_frames / meta_data["fps"]
        )
    ...
    def map_single_frame_id(
        self, new_sequence_id: int, raw_frame_rate: float, total_raw_frames: int
    ) -> int:
        if not self.fix_frame_rate:
...
class LoadVideo(DataProcessingOperator, FrameSamplerByRateMixin):
    ...
    def __call__(self, data: str):
        ...
        for frame_id in range(num_frames):
            frame_id = self.map_single_frame_id(
                frame_id, raw_frame_rate, total_raw_frames
            )
